...
client.ws.on("INTERACTION_CREATE", async interaction => {
	try {
		if (client.slashcommands.get(interaction.data.name)) {
			commandLog(`${interaction.member.user.username}#${interaction.member.user.discriminator} (\`${interaction.member.user.id}\`) ran slash command \`${interaction.data.name}\` in the \`${interaction.channel_id}\` channel of \`${interaction.guild_id}\``, { client, content: "" });
